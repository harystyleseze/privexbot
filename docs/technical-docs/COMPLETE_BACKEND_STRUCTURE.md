...
└── src/
    └── app/
        ...
        │   └── session.py
        │
        ├── utils/                       ✅ EXISTS (needs additions)
        │   ├── __init__.py
        │   ├── redis.py                 ✅ EXISTS
        │   ├── file_storage.py          ❌ MISSING - File upload/download utilities
        │   ├── chunking_utils.py        ❌ MISSING - Text chunking helpers
        │   └── validation.py            ❌ MISSING - Common validation functions
        │
        ├── models/                      ✅ PARTIAL (needs additions)
        │   ├── __init__.py
        │   ├── organization.py          ✅ EXISTS
        │   ├── organization_member.py   ✅ EXISTS
        │   ├── workspace.py             ✅ EXISTS
        │   ├── workspace_member.py      ✅ EXISTS
        │   ├── user.py                  ✅ EXISTS
        │   ├── auth_identity.py         ✅ EXISTS
        │   ├── chatbot.py               ✅ EXISTS (may need deployment field update)
        │   ├── chatflow.py              ✅ EXISTS (may need deployment field update)
        │   ├── knowledge_base.py        ✅ EXISTS
        │   ├── document.py              ✅ EXISTS (has annotations field)
        │   ├── chunk.py                 ✅ EXISTS
        │   ├── api_key.py               ✅ EXISTS
        │   ├── lead.py                  ✅ EXISTS
        │   ├── chat_session.py          ❌ MISSING - Chat history sessions
        │   ├── chat_message.py          ❌ MISSING - Individual messages
        │   ├── credential.py            ❌ MISSING - Chatflow node credentials
        │   └── deployment.py            ❌ MISSING - Multi-channel deployments (optional, can use JSONB)
        │
        ├── schemas/                     ✅ PARTIAL (needs additions)
        │   ├── __init__.py
        │   ├── organization.py          ✅ EXISTS
        │   ├── workspace.py             ✅ EXISTS
        │   ├── user.py                  ✅ EXISTS
        ...
        │   ├── document.py              ✅ EXISTS
        │   ├── chunk.py                 ✅ EXISTS
        │   ├── chat.py                  ❌ MISSING - Chat request/response schemas
        │   ├── lead.py                  ❌ MISSING - Lead capture schemas
        │   ├── credential.py            ❌ MISSING - Credential schemas
        │   └── draft.py                 ❌ MISSING - Draft mode schemas
        │
        ├── services/                    ✅ PARTIAL (many missing)
        │   ├── __init__.py
        │   ├── auth_service.py          ✅ EXISTS
        │   ├── tenant_service.py        ✅ EXISTS
        │   ├── permission_service.py    ✅ EXISTS
        │   ├── embedding_service.py     ✅ EXISTS
        │   ├── vector_store_service.py  ✅ EXISTS
        │   │
        │   ├── draft_service.py         ❌ MISSING - CRITICAL - Unified draft management
        │   ├── kb_draft_service.py      ❌ MISSING - CRITICAL - KB draft operations
        │   ├── chatbot_service.py       ❌ MISSING - CRITICAL - Chatbot execution
        │   ├── chatflow_service.py      ❌ MISSING - CRITICAL - Chatflow orchestration
        │   ├── chatflow_executor.py     ❌ MISSING - CRITICAL - Node-by-node execution
        │   ├── inference_service.py     ❌ MISSING - CRITICAL - Secret AI integration
        │   ├── session_service.py       ❌ MISSING - Chat history management
        │   ├── credential_service.py    ❌ MISSING - Credential encryption/management
        │   ├── geoip_service.py         ❌ MISSING - IP geolocation
        │   ├── document_processing_service.py ❌ MISSING - Parse, chunk, embed pipeline
        │   ├── chunking_service.py      ❌ MISSING - Chunking strategies
        │   ├── indexing_service.py      ❌ MISSING - Vector indexing
        │   └── retrieval_service.py     ❌ MISSING - Search with annotation boosting
        │
        ├── integrations/                ❌ FOLDER MISSING - All files needed
        │   ├── __init__.py
        │   ├── telegram_integration.py  ❌ MISSING - Telegram Bot API
        │   ├── discord_integration.py   ❌ MISSING - Discord webhook
        │   ├── whatsapp_integration.py  ❌ MISSING - WhatsApp Business API
        │   ├── zapier_integration.py    ❌ MISSING - Zapier webhook
        │   ├── crawl4ai_adapter.py      ❌ MISSING - Website scraping (LLM-ready)
        │   ├── firecrawl_adapter.py     ❌ MISSING - Alternative scraper
        │   ├── jina_adapter.py          ❌ MISSING - Jina Reader
        │   ├── google_adapter.py        ❌ MISSING - Google Docs/Sheets
        │   ├── notion_adapter.py        ❌ MISSING - Notion API
        │   └── unstructured_adapter.py  ❌ MISSING - Document parsing
        │
        ├── chatflow/                    ❌ FOLDER MISSING - All files needed
        │   ├── __init__.py
        │   ├── nodes/
        │   │   ├── __init__.py
        │   │   ├── base_node.py         ❌ MISSING - Base class for all nodes
        │   │   ├── llm_node.py          ❌ MISSING - LLM call node
        │   │   ├── kb_node.py           ❌ MISSING - Knowledge base search
        │   │   ├── condition_node.py    ❌ MISSING - If/else branching
        │   │   ├── http_node.py         ❌ MISSING - HTTP request
        │   │   ├── variable_node.py     ❌ MISSING - Set/get variables
        │   │   ├── code_node.py         ❌ MISSING - Custom Python code
        │   │   ├── memory_node.py       ❌ MISSING - Save/load memory
        │   │   ├── database_node.py     ❌ MISSING - Database query/insert
        │   │   ├── loop_node.py         ❌ MISSING - For-each iteration
        │   │   └── response_node.py     ❌ MISSING - Final response
        │   └── utils/
        │       ├── __init__.py
        │       ├── variable_resolver.py ❌ MISSING - Resolve {{variables}}
        │       └── graph_builder.py     ❌ MISSING - Build execution graph
        │
        ├── tasks/                       ✅ PARTIAL (needs additions)
        │   ├── __init__.py
        │   ├── celery_worker.py         ✅ EXISTS (may need config updates)
        │   ├── document_tasks.py        ❌ MISSING - Document processing tasks
        │   ├── crawling_tasks.py        ❌ MISSING - Website crawling
        │   └── sync_tasks.py            ❌ MISSING - Cloud sync (Notion, Google)
        │
        ├── api/v1/
        │   ├── __init__.py
        │   ├── dependencies.py          ✅ EXISTS
        │   └── routes/                  ✅ PARTIAL (many missing)
        │       ├── __init__.py
        │       ├── auth.py              ✅ EXISTS (empty)
        │       ├── org.py               ✅ EXISTS (empty)
        │       ├── workspace.py         ✅ EXISTS (empty)
        │       ├── chatbot.py           ✅ EXISTS (empty - needs full implementation)
        │       ├── chatflows.py         ❌ MISSING - Chatflow CRUD + draft endpoints
        │       ├── knowledge_bases.py   ❌ MISSING - KB CRUD + draft endpoints
        │       ├── documents.py         ❌ MISSING - Document management
        │       ├── chunks.py            ❌ MISSING - Chunk operations
        │       ├── credentials.py       ❌ MISSING - Credential management
        │       ├── leads.py             ❌ MISSING - Lead dashboard
        │       ├── public.py            ❌ MISSING - CRITICAL - Public chat API
        │       └── webhooks/            ❌ FOLDER MISSING
        │           ├── __init__.py
        │           ├── telegram.py      ❌ MISSING - Telegram webhook handler
        │           ├── discord.py       ❌ MISSING - Discord webhook handler
        │           └── whatsapp.py      ❌ MISSING - WhatsApp webhook handler
        │
        ├── auth/                        ✅ EXISTS
        │   ├── __init__.py
        │   └── strategies/
